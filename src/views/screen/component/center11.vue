...
<script>
import { CenterClients } from '@/api/screen.js'
...
export default {
  ...
  methods: {
    async initCharts() {
      ...
      // 判断ips是不是null，如果不是 ，就得获取
      if (allArr.ips !== null) {
        newAdd.name = allArr.ips
        newAdd.value = [...(allArr.value[0] + space), ...(allArr.value[1] + 17)]
        return newAdd
      }
      ...
      let newLine = [
        {
          coord: ['30.80', '48.20'],
          effect: { color: '#36fff6' },
          lineStyle: { color: '#0BC6FE', curveness: 0 }
        },
        {
          coord: ['24.80', '31.20']
        }
      ...
      if (allArr.ips !== null) {
        newLine[0].coord = newAdd.value
        newLine[1].coord = [...allArr.value[0], ...allArr.value[1]]
        return newLine
      }
